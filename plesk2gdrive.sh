...
currentday="$(LC_TIME="en_GB.UTF-8" date +"%A")-$(hostname)"
...
plesk2gdrive="plesk2gdrive-$(hostname)"
...
declare -a FOLDERINFO=($(~/drive-linux-$ARCH list -q "name = '$plesk2gdrive' and  mimeType='application/vnd.google-apps.folder'" --no-header))
...
while getopts "c:d:vrlh" opt; do
  case $opt in
    ...
    echo -e ${yellow}
    echo -e "$(~/drive-linux-$ARCH info $OPTARG)"
    echo -e ${red}
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]
        then
        echo DELETING FILE!
        ~/drive-linux-$ARCH delete $OPTARG
    fi
    echo -e ${nocolor}
    ...
    ~/drive-linux-$ARCH list -q "name='$currentday.tar'"
    ;;
    (r) #Performs backup.
    ...
    declare -i backups=$(~/drive-linux-$ARCH list -q "name='$currentday.tar'" --no-header | tee /dev/null |(wc -l))
    ...
    while [ ! -z  $firstbackup ]; do
        ...
        ~/drive-linux-$ARCH delete $firstbackup
        RESULT=($(~/drive-linux-$ARCH list -q "name='$currentday.tar'" --no-header))
        firstbackup=${RESULT[0]}
    done
    echo -e "${yellow}Copying file to Google Drive.${nocolor}"
    #Uploading backup to Google Drive
    ~/drive-linux-$ARCH upload -p $FOLDERID $backuplocation/$currentday.tar
    ...
    declare -i RESULT=($(~/drive-linux-$ARCH list -q "name='$currentday.tar'" --no-header))
    declare -i backups=$(~/drive-linux-$ARCH list -q "name='$currentday.tar'" --no-header | tee /dev/null |(wc -l))
    echo -e "${yellow}Total Backups for today?: ${red}$backups${nocolor}"
    declare -i addresult=1
    declare -i COUNTER=0
    ...
    echo
    echo -e "This script was developed by Brian Aldridge and modified by MilloLab. For updates and contact information please visit http://www.BiteOfTech.com"
     ;;
